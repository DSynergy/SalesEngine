...
class InvoiceItemsRepoTest < Minitest::Test
  ...
  def setup
    ...
    @invoice_items_repository = InvoiceItemsRepository.build_invoice_items(@invoice_items_data)
  end

  def test_it_can_load_a_repo
    assert_equal 24, @invoice_items_repository.invoice_items.length
  end

  def test_it_returns_a_random_invoice_item
    random_invoice_item1 = @invoice_items_repository.invoice_items.random
    random_invoice_item2 = @invoice_items_repository.invoice_items.random
